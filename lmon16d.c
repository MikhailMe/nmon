...
/* ЕСЛИ НА ЭКРАНЕ ВЫВОД БОЛЕЕ, ЧЕМ ПО 100 ДИСКАМ*/
...
/* увелечение @oldptr на @size байт памяти */
...
typedef struct nvmlDevice_st *nvmlDevie_t;
...
/* Вырезаем все после первого пробела в колбеке
 * Удалить все '&' непосредственно перед пробелом
 */
...
/* Удалите вывод ошибки в этот буфер и отобразите его, если NMONDEBUG = 1 */
char errorstr[70];
int error_on = 0;
void error(char *err)
{
...
/* 
lscpu собирает информацию о архитектуре процессора из sysfs, /proc/cpuinfo и любых применимых библиотек
*/
...
#define NUMBER_COL 23
	...
	/* есть файл usr/bin/lscpu и заполняем структуру lscpu из файла /usr/bin/lscpu */
...
int proc_cpu_done = 0;		/* Флаг, если мы запустили функцию proc_cpu () уже в этом интервале */
...
struct {
    ...
    char *buf;
    int read_this_interval;	/* отслеживать обновления для каждого обновления,
    						 чтобы остановить двойной сбор данных */
} proc[P_NUMBER];


/* будем брать информацию из этих файлов */
void proc_init()
{
...
void proc_read(int num)
{
    ...
    /* Мы перечитываем P_STAT, теперь флаг proc_cpu(), что он должен перепрограммировать эти данные */
    if (num == P_STAT)
	proc_cpu_done = 0;

    if (proc[num].size == 0) {
	/* первый раз, поэтому выделяем изначально */
	proc[num].buf = MALLOC(512);
	proc[num].size = 512;
    }

    for (i = 0; i < 4096; i++) {	/* ПРЕДУПРЕЖДЕНИЕ MAGIC COOKIE POWER8 по умолчанию может иметь 2655 процессов*/
    ...
    if (num == P_VERSION) {
	found = 0;
	for (i = 0; i < size; i++) {	/* удалите некоторые странные вещи, которые нашли трудный путь в различных версиях Linux и драйверах устройств */
	    if (found == 0 &&
		proc[num].buf[i] == ')' &&
		proc[num].buf[i + 1] == ' ' &&
		proc[num].buf[i + 2] == '(') {
	...
	if (proc[num].buf[i] == '\t') {
	    ...
	    /* заменить символы новой строки null */
	    proc[num].lines++;
	    proc[num].buf[i] = '\0';
    ...
    /* Установили флаг, чтобы мы не перечитывали данные, даже если их вызывали несколько раз в одном интервале */
...
struct procsinfo {
    ...
    /* Информация о сигнале здесь устарела. */
...
#define MVPRINTW(row,col,string) {move((row),(col)); \
					...
					printw(string); \
					attroff(A_STANDOUT); }
FILE *fp;			/* указатель файла для вывода электронных таблиц */
...
/* глобальное название программы для его печати */
char *progname;

/*****************************************************************************************************************/
/* Основная структура данных для собранной статистики.
 * Две версии - это предыдущие и текущие данные.
 * Часто это различие, которое печатается.
 * Указатели сменяются - текущий становится прежним
 * а предыдущий - над написанным, а не перемещением данных.
 */

/* изменил порядок здесь, чтобы соответствовать ядру лет (man 5 /proc/stat) */
...
/* Сброс в конце каждого интервала, так что LPAR cfg считывается только раз в каждый интервал
  * даже если proc_lparcfg () вызывается несколько раз
  * Примечание: lparcfg не читается через proc_read ()!
*/
...
/* Возврат 0 означает, что данные недоступны */
int proc_lparcfg()
{
    static FILE *fp = (FILE *) - 1;
/* Попробуйте только прочитать /proc/ppc64/lparcfg - помните, если он доступен для чтения */
    static int lparinfo_not_available = 0;
    int i;
    char *str;
    /* Если мы уже читаем и обрабатываем / proc / lparcfg в этом интервале - просто return*/
	...
	if ((fp = fopen("/proc/ppc64/lparcfg", "r")) == NULL) {
		...
		/* Эвристика для определения PowerKVM Host
		a) внутри ifdef POWER, поэтому не может быть x86
		b) / proc / ppc64 / lparcfg отсутствует
		c) / etc / * файлы с легкостью имеют подсказки
		Подтверждено для IBM_POWERKVM 2.1
		*/
...
#define GETDATA(variable) lparcfg.variable = read_longlong( __STRING(variable) );
    ...
    /* Грубая сила, может предоставлять временные неверные данные во времясобытий динамической реконфигурации
     */
...
int read_vmstat()
{
    ...
    /* Примечание:
    если запрошенная переменная не найдена
    в /proc/vmstat, тогда она установлена в -1 */
    GETVM(nr_dirty);
    GETVM(nr_writeback);
    GETVM(nr_unstable);
...
/* Эти макросы упрощают доступ к основной структуре данных */
...
void get_cpu_cnt()
{
    ...
    /* Получить информацию о CPU из /proc/stat и заполнить proc[P_STAT] */
    proc_read(P_STAT);

    /* Начните с индекса [1], поскольку [0] содержит общую статистику CPU */
...
/* Это экономит общее использование процессора для последующего построения графика на экране */
...
void init_pairs()
{
 ...
 /* Обработчик сигналов
  * SIGUSR1 или 2 используется для остановки nmon cleanly
  * SIGWINCH используется, когда изменяется размер окна
  */
...
/* Поиск правильной строки состояния процесса */
...
#ifdef GETUSER
/* Преобразование идентификатора пользователя (UID) в имя с кешированием для скорости
 * getpwuid () должен быть безопасным для NFS / желтых страниц
 */
...
/* Пользовательские группы дисков */
...
/* Мы заказываем этот массив, а не фактические таблицы процессов
 * индекс - это позиция в таблице процессов и
 * размер - это память, используемая в байтах
 * io - это ввод-вывод хранилища, выполненный за последний период в байтах
 * время - это процессор, используемый в последнем периоде в секундах
 */
struct topper {
    int index;
    int other;
...
// Такая, как частота процессора, количество тактов,
// количество сброшенных страниц памяти на диск,
//количество системных прерываний, время загрузки и т д.
...
/* Управление внешними сборщиками данных */
